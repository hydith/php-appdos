...
<div class="main-panel">
        ...
        <div class="col-sm-12" style="background: #ecf0f1;"> 
            <h3 style="text-align:center; margin-bottom:50px;">FORM PERNYATAAN<br>   MELAKSANAKAN PENELITIAN </h3>
            ...
            <div class="panel panel-default" style="border-left: 6px solid #dd5209;">
                <div class="panel-heading" style="color:#dd5209"><b>Form Pengisian Kegiatan</b></div>
                    <div class="panel-body">
                          ...
                          <?php echo form_open_multipart('keloladosen/submitkegiatanpenelitian');?>
       ...
       <input type="hidden" name="id" id="id" value="<?php echo $this->input->get('idpengajuan');?>"/> 
         ...
         <td>
         <label>Bentuk Karya Ilmiah</label>
         ...
         <tr>
         <td>
         <label>Status Penulis</label>
         <div id="comboxstatus"> <!-- sebagai indentitas combo box -->
              <select name="penulis" id="penulis" class="form-control" required>
                 <option></option>
                  <option value="mandiri">Mandiri</option>
                  <option value="utama">Utama/Ketua</option>
        ...
        <tr id="anggota" style="display:none">
         <td>
         <label>Jumlah Anggota</label>
         <div id="combox"> <!-- sebagai indentitas combo box -->
             <input type="text" name="jml_anggota" id="jml_anggota" class="form-control" style="width:75px;" >
          </div>
         </td>
         </tr>
         ...
         <div id="combox"> <!-- sebagai indentitas combo box -->
             <input readonly="true" type="text" name="nilai" value="0" id="nilai" class="form-control" style="width:75px;" required>
             <small id="Hint" class="form-text text-muted" style="color:#f9243f;">Ket : </small>
             <small id="hintstatus" class="form-text text-muted" style="color:#f9243f;"></small>
...
$(document).ready(function () {
    $('#combox1 select').change(function () {
      ...
      if(kodeKeg=="III.A"){
        $.ajax({
            url: "<?=base_url()?>index.php/keloladosen/formgeturaiankegiatanpenelitiankaryailmiah/",       //memanggil function controller dari url
            async: false,
        ...
        $.ajax({
            url: "<?=base_url()?>index.php/keloladosen/formgeturaiankegiatanpenelitian/",       //memanggil function controller dari url
            async: false,
            type: "POST",     //jenis method yang dibawa ke function
...
$(document).ready(function () {
    $('#comboxstatus select').change(function () {
       var status = $(this).val();
        if(status=="mandiri"){
            var jumlah=kreditpenelitian;
            $('#nilai').val(jumlah);
            $('#hintstatus').html("Penulis Mandiri 100% dari Angka Kredit");
            
        }
        else if(status=="utama"){
            var jumlah=(60/100) *kreditpenelitian;
            $('#nilai').val(jumlah);
            $('#hintstatus').html("Penulis Utama 60% dari Angka Kredit");
        }
        else{
            $('#jml_anggota').change(function () {
            var jmlanggota = $('#jml_anggota').val(); 
            var len = jmlanggota.length;     
            if(len>=1){
            var jumlah=(40/100) * kreditpenelitian / jmlanggota;
            $('#hintstatus').html("Penulis Anggota 40% dari Angka Kredit dan dibagi jumlah anggota");
             $('#nilai').val(jumlah);
            }
...
$(document).ready(function () {
    $('#combox2 select').change(function () {
        ...
        if(kodeKeg=='III.A'){
     $.ajax({
            url: "<?=base_url()?>index.php/keloladosen/formgetbentukkaryailmiah/",       //memanggil function controller dari url
            async: false,
            type: "POST",     //jenis method yang dibawa ke function
            ...
            $.ajax({
            url: "<?=base_url()?>index.php/keloladosen/formgeturaiankegiatanpenelitiandetail/",       //memanggil function controller dari url
            async: false,
            type: "POST",     //jenis method yang dibawa ke function
            data: "noKeg="+no,   //data yang akan dibawa di url
            dataType: "json",
            success: function(data) {
                kreditpenelitian = data['AngkaKredit'];
    ...
    $('#combox3 select').change(function () {
        var kodeKeg = $(this).val();
        $.ajax({
            url: "<?=base_url()?>index.php/keloladosen/formgetmacamkaryailmiah/",       //memanggil function controller dari url
            async: false,
            type: "POST",     //jenis method yang dibawa ke function
            data: "kode="+kodeKeg,   //data yang akan dibawa di url
            dataType: "html",
            success: function(data) {
                $('#golongan').html(data);   //hasil ditampilkan pada combobox id=kota
            },
    ...
    $('#combox4 select').change(function () {
        var no = $(this).val();
       $.ajax({
            url: "<?=base_url()?>index.php/keloladosen/formgetmacamkaryailmiahdetail/",       //memanggil function controller dari url
            async: false,
            type: "POST",     //jenis method yang dibawa ke function
